...
module.exports = class MultimediaProfile {
  constructor (params) {
    ...
    this.params = params
    this.baseUrl = `https://config-service.produs1.ciscoccservice.com/cms/api/organization/${this.params.orgId}/multimedia-profile`
  }
  ...
  /**
  * Gets full list of multimedia profiles
  * @return {Promise} the fetch promise, which resolves to multimedia profiles JSON array when
  * successful
  */
   async listAll () {
    try {
   ...
   */
  update (body) {
    // console.log('update multimedia profile', body)
    body.id = body.id || body.multimediaProfileId
